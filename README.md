...
Canonical team names, aliases, and region attributions live in `lib/teams.generated.ts` — produced by `npm run sync:teams`, which queries the [Leaguepedia Cargo API](https://lol.fandom.com/wiki/Special:CargoTables) for every team active this season in the tracked leagues (LCK, LEC, LCS, LPL, CBLOL). The runtime never hits the Leaguepedia API; the generated file is committed.

Refresh whenever rosters change (rebrand, merger, promotion/relegation):

```bash
npm run sync:teams
```

The Fandom API rate-limits anonymous queries aggressively, so a sync takes ~30–60 seconds and may need a retry from a different IP if you've been probing it recently.
...
### Quota, caching, and abuse protection

YouTube Data API v3 allows 10,000 units/day. Each monitored channel costs 1 unit per fetch. The `/api/feed` route is auth-gated (returns 401 without a session), caches results for 3 minutes in-memory, and applies a 20 req/min per-user rate limit. The `/watch/[id]` route validates the YouTube video ID format before rendering an embed.
...
lib/
  ...
  team-aliases.ts             # extractTeamsFromTitle (reads teams.generated)
  teams.generated.ts          # Generated by sync:teams — DO NOT hand-edit
...
scripts/
  sync-teams.ts               # Pulls Leaguepedia → lib/teams.generated.ts
